//插件编写
(function($) {
    $.fn.wfdesigner = function() {
        var wf_view;
        var nodelist = new Array();
        var rulelist = new Array();

        var wfnode = function(options) {
            this.key = "";
            this.text = "";
            this.node = undefined;
            ...
            if (wf_view == null || wf_view == undefined) {
                wf_view = Raphael(this.attr("id"), $(window).width(), $(window).height() - 28);
            }
        ...
        var getArrForString = function(x1, y1, x2, y2, size) {
                var angle = Raphael.angle(x1, y1, x2, y2);
                var a45 = Raphael.rad(angle - 45);
                var a45m = Raphael.rad(angle + 45);
                var x2a = x2 + Math.cos(a45) * size;
            ...
            //求规则和节点的交点
        var getnodeline = function(wnode, wrule) {
                var x = wnode.x;
                var y = wnode.y;
            ...
            //求规则和节点的交点
        var nodeIncludePoint = function(wnode, point) {
                var x = wnode.x;
                var y = wnode.y;
                var height = wnode.settings.nodeHeight;
                var width = wnode.settings.nodeWidth;
